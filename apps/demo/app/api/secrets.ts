export const API_KEY = "ce453b1a-3687-43c2-943a-8ec39999d2c4";
export const API_URL = "https://api.liveavatar.com";
export const AVATAR_ID = "19955cd3-3612-4899-9467-554867c9d461";

// FULL MODE Customizations
// Wayne's avatar voice and context
export const VOICE_ID = "864a26b8-bfba-4435-9cc5-1dd593de5ca7";
export const CONTEXT_ID = "66295286-ab9d-4119-8983-ce52f285c29f";
export const LANGUAGE = "pt";

// CUSTOM MODE Customizations
export const ELEVENLABS_API_KEY = "YOUR_ELEVENLABS_API_KEY";
export const OPENAI_API_KEY = "YOUR_OPENAI_API_KEY";
